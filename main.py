...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS activations (
        ...
        message_id TEXT,
        UNIQUE(mobile, app) ON CONFLICT REPLACE
    )
    ''')
...
async def is_duplicate(app, mobile, email=None):
    """Check for any existing activation regardless of status"""
    try:
        conditions = ["app = ?", "mobile = ?"]
        params = [app.lower(), mobile.replace(" ", "")]
        ...
        logger.error(f"Error in is_duplicate: {e}")
        return None

async def write_activation(email, app, mobile, status="pending", reason="pending"):
    """Write or update activation with better error handling"""
    if not all([email, app, mobile]):
        return False, "Missing required fields"
        
    if not re.fullmatch(r'\d{10}', mobile.replace(" ", "")):
...
async def update_activation(email, app, mobile, status, reason="0"):
    ...
    try:
        ...
        if not updated:
            logger.warning(f"No activation found to update: {app}, {mobile}, {email}")
            return False
...
async def process_activation_proof(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        existing_status = await is_duplicate(app, mobile, email)
        if existing_status:
            if existing_status == 'pending':
                await update.message.reply_text(
                    f"⏳ *Activation Already Pending*\n\n"
                    f"This mobile number already has a pending activation for {app.upper()}",
                    parse_mode='Markdown'
                )
                return MAIN_MENU
                ...
                await update.message.reply_text(
                    f"✅ *Already Approved*\n\n"
                    f"This mobile number is already approved for {app.upper()}",
                    parse_mode='Markdown'
                )
                return MAIN_MENU

        # Record activation
        success, message = await write_activation(email, app, mobile)
        if not success:
            await update.message.reply_text(f"❌ {message}")
            return MAIN_MENU

        # Forward to channel with enhanced error handling
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    await execute_db_query(
                        "UPDATE activations SET message_id = ? WHERE mobile = ? AND app = ?",
                        (message.message_id, mobile.replace(" ", ""), app),
                        commit=True
                    )
            ...
            logger.error(f"Failed to forward to channel after {max_retries} attempts: {channel_error}")
            ...
            if admin_id:
                try:
                    await context.bot.send_message(
                        chat_id=admin_id,
                        text=f"❌ Failed to forward activation to channel:\n\n"
                             f"App: {app}\n"
                             f"User: {email}\n"
                             f"Mobile: {mobile}\n"
                             f"Error: {str(channel_error)[:200]}"
                    )
                except Exception as admin_error:
                    logger.error(f"Failed to notify admin: {admin_error}")

            # Clean up failed activation record
            await execute_db_query(
                "DELETE FROM activations WHERE mobile = ? AND app = ? AND status = 'pending'",
                (mobile.replace(" ", ""), app),
                commit=True
            )

            await update.message.reply_text(
                "❌ *Failed to submit activation*\n\n"
                "Our system encountered an error. Please try again later.",
                parse_mode='Markdown'
            )
            return MAIN_MENU
        ...
        logger.error(f"Error in process_activation_proof: {e}", exc_info=True)
        await update.message.reply_text(
